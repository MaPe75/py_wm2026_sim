...
# ===========================================================================
# Margin-Analyse: Wo tendiert das Modell zu 2+ Toren Unterschied?
# Zeigt je Spiel die Siegmargen des Favoriten und vergleicht den
# Erwartungswert der Tipp-Alternativen 1:0 / 2:0 / 2:1, damit man sieht,
# wo ein "spektakulaerer" Tipp (fast) gratis ist.
# ===========================================================================
def _matrix(a, b):
    ra = RATING[a] + HOME_BONUS.get(a, 0)
    rb = RATING[b] + HOME_BONUS.get(b, 0)
...
def print_margin_analysis():
    ...
    for g, a, b in MD2_FIXTURES:
        ...
        p1 = margin(1)
        p2plus = sum(margin(d) for d in range(2, MAX_GOALS + 1))
        pwin = p1 + p2plus

        def tip_fav(hf, af):
            return (hf, af) if fav == a else (af, hf)

        e10, e20, e21 = _ep(tip_fav(1, 0), P), _ep(tip_fav(2, 0), P), _ep(tip_fav(2, 1), P)
        rows.append((g, fav, dog, pwin, p1, p2plus, e10, e20, e21))

    rows.sort(key=lambda r: r[5], reverse=True)   # nach P(>=2 Tore)
    print("=" * 86)
    print("  MARGIN-ANALYSE  (sortiert nach Wahrscheinlichkeit fuer 2+ Tore Unterschied)")
    print("=" * 86)
    print(f"{'Gr':<3}{'Favorit -> Underdog':<30}{'P(Sieg)':>8}{'P(+1)':>7}{'P(>=2)':>8}"
          f"   {'E[1:0]':>7}{'E[2:0]':>7}{'E[2:1]':>7}")
    print("-" * 86)
    for g, fav, dog, pwin, p1, p2plus, e10, e20, e21 in rows:
        # Tipp-Empfehlung: 2:0 nur, wenn fast gratis (Abstand < 0.03 zum 1:0)
        upgrade = " <= 2:0 fast gratis" if (e10 - e20) < 0.03 and p2plus > 0.35 else ""
        print(f"{g:<3}{fav+' -> '+dog:<30}{100*pwin:>7.0f}%{100*p1:>6.0f}%{100*p2plus:>7.0f}%"
              f"   {e10:>7.2f}{e20:>7.2f}{e21:>7.2f}{upgrade}")
    print("\n  Lesehilfe: P(+1)=Sieg mit genau 1 Tor, P(>=2)=Sieg mit 2+ Toren.")
    print("  E[2:1] liegt fast immer ~0.01 unter E[1:0] -> spektakulaerer Tipp,")
    print("  praktisch ohne Erwartungswert-Verlust. E[2:0] lohnt nur oben.")
    print()
